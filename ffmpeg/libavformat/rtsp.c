...
#endif
    /**
     * Begin Add by H.Kernel for rtsp play scale and speed
     */
    { "scale", "set rtsp play command scale arg", OFFSET(play_scale), AV_OPT_TYPE_STRING, {.str = NULL}, 0, 0, DEC },
    { "speed", "set rtsp play command speed arg", OFFSET(play_speed), AV_OPT_TYPE_STRING, {.str = NULL}, 0, 0, DEC },
    /**
     * End Add by H.Kernel for rtsp play scale and speed
     */
    { NULL },
};
                       ...
                       int return_on_interleaved_data, const char *method)
{
    ...
    if(0 < rt->tcp_recv_size ) {
        av_log(s, AV_LOG_DEBUG, "read reply from buffer,size:%d\n", rt->tcp_recv_size);
        return ff_rtsp_read_reply_from_buffer(s,reply,content_ptr,return_on_interleaved_data,method);
    }
...
/**
 * add  by H.Kernel 2018-07-31 for optimize rtsp tcp recv
 * Receive one RTP packet from an TCP interleaved RTSP stream.
 */

#define RETRY_MAX_COUNT 10
int ff_rtsp_read_reply_from_buffer(AVFormatContext *s, RTSPMessageHeader *reply,
                       unsigned char **content_ptr,
                       int return_on_interleaved_data, const char *method)
{
    RTSPState *rt = s->priv_data;
    int len,i,ret;
    char rtsp_msg[4096], word[1024];
    const char *p,*start,*end;
    int line_count = 0, line_size = 0,request = 0;

    int attampt = 0;
    /* 1. try recv more data on this tcp connection */
    if(0 == rt->tcp_recv_size ) {
reread:
        if(RETRY_MAX_COUNT < attampt) {
            return 0;
        }
        len = RTSP_TCP_RECV_MAX_SIZE - rt->tcp_recv_size;
        ret = ffurl_read(rt->rtsp_hd, &rt->tcp_recv_buf[rt->tcp_recv_size], len);
        if (ret < 0) {
            av_log(s, AV_LOG_ERROR, "read reply from buffer,read socket fail,size:[%d] len:[%d].\n",
                                       rt->tcp_recv_size, len);
            return ret;
        }

        rt->tcp_recv_size += ret;
        attampt++;
    } 

    /* check the interleaved $ */
    for(;;) {
        if(0 == rt->tcp_recv_size ) {
            goto reread;
        }
        /* 2.deal the interleaved packet */
        if('$' != rt->tcp_recv_buf[0]) {
            break;
        }
        if(return_on_interleaved_data) {
            return 0; /* redo deal the $ */
        }
        ret = ff_rtsp_skip_packet_from_buffer(s);
        if(0 != ret) {
            av_log(s, AV_LOG_ERROR, "read reply from buffer,skip the rtp packet fail,size:[%d].\n",
                                       rt->tcp_recv_size);
            return ret;
        }
    }

    /* deal the rtsp packet */
    start = &rt->tcp_recv_buf[0];

    end = av_strnstr(start,"\r\n\r\n",rt->tcp_recv_size);
    if(NULL == end) {
        /* need read more data */
        goto reread;
    }
    ...
    for (i = 0;i < len; i++) {
        ...
        line_size = end - start + 3;
        av_strlcpy(rtsp_msg,start,line_size);
        start = end + 2;
        ...
        if (line_count == 0) {
            ...
            /* check the rstp start line */
            if((0 != strncmp(word, "RTSP/", 5))
                &&(0 != strncmp(word, "OPTIONS", 7))
                &&(0 != strncmp(word, "ANNOUNCE", 8))
                &&(0 != strncmp(word, "DESCRIBE", 8))
                &&(0 != strncmp(word, "SETUP", 5))
                &&(0 != strncmp(word, "GET_PARAMETER", 13))
                &&(0 != strncmp(word, "SET_PARAMETER", 13))
                &&(0 != strncmp(word, "PLAY", 4))
                &&(0 != strncmp(word, "RECORD", 6))
                &&(0 != strncmp(word, "PAUSE", 5))
                &&(0 != strncmp(word, "REDIRECT", 8))
                &&(0 != strncmp(word, "TEARDOWN", 8))) {

                av_log(s, AV_LOG_ERROR, "[rtsp]:ff_rtsp_read_reply_from_buffer,this:[%s] is not rtsp packet\n",word);

                return AVERROR_EOF;
            }
            if (!strncmp(word, "RTSP/", 5)) {
                get_word(word, sizeof(word), &p);
                reply->status_code = atoi(word);
                av_strlcpy(reply->reason, p, sizeof(reply->reason));
            } else {
                av_strlcpy(reply->reason, word, sizeof(reply->reason)); // method
                get_word(word, sizeof(word), &p); // object
                request = 1;
            }
        } else {
            ff_rtsp_parse_line(s, reply, p, rt, method);
    ...
    if (reply->content_length > 0) {
        ...
        int content_length = reply->content_length;
        /* leave some room for a trailing '\0' (useful for simple parsing) */
        char* content = av_malloc(content_length + 1);
        if (!content)
            return AVERROR(ENOMEM);
        memcpy(content, &rt->tcp_recv_buf[len], content_length);
        content[content_length] = '\0';
        ...
        len += reply->content_length;
    }

    if(len > rt->tcp_recv_size) {
        /* need more data */
        return 0;
    }
    if(len <= rt->tcp_recv_size) {
        /* skip the rtsp packet */
        rt->tcp_recv_size -= len;
        memmove(&rt->tcp_recv_buf[0],&rt->tcp_recv_buf[len],rt->tcp_recv_size);
    }

    if (request) {
        char base64buf[AV_BASE64_SIZE(sizeof(rtsp_msg))];
        const char* ptr = rtsp_msg;

        if (!strcmp(reply->reason, "OPTIONS")) {
            snprintf(rtsp_msg, sizeof(rtsp_msg), "RTSP/1.0 200 OK\r\n");
            if (reply->seq)
                av_strlcatf(rtsp_msg, sizeof(rtsp_msg), "CSeq: %d\r\n", reply->seq);
            if (reply->session_id[0])
                av_strlcatf(rtsp_msg, sizeof(rtsp_msg), "Session: %s\r\n",
                                              reply->session_id);
        } else {
            snprintf(rtsp_msg, sizeof(rtsp_msg), "RTSP/1.0 501 Not Implemented\r\n");
        }
        av_strlcat(rtsp_msg, "\r\n", sizeof(rtsp_msg));

        if (rt->control_transport == RTSP_MODE_TUNNEL) {
            av_base64_encode(base64buf, sizeof(base64buf), rtsp_msg, strlen(rtsp_msg));
            ptr = base64buf;
        }
        ffurl_write(rt->rtsp_hd_out, ptr, strlen(ptr));
...
int ff_rtsp_connect(AVFormatContext *s)
{
    ...
    /** add by H.Kernel 2018-06-19 for HIK rtsp server
     * options reply 311 error without location
     * and try DESCRIBE while reply with redirect location
     */
    int send_rtsp_options   = 1;
    /** add by H.Kernel 2018-07-16 for HIK rtsp server
     * options for rtsp tcp send recv buffer size
     */
    AVDictionary *opts      = NULL;
...
redirect:
    ...
    if (rt->control_transport == RTSP_MODE_TUNNEL) {
         ...
         */
        if (0 < rt->buffer_size) {
            av_dict_set_int(&opts, "recv_buffer_size", rt->buffer_size, 0);
            av_dict_set_int(&opts, "send_buffer_size", rt->buffer_size, 0);
        }else {
            av_dict_set_int(&opts, "recv_buffer_size", 2048000, 0);
            av_dict_set_int(&opts, "send_buffer_size", 2048000, 0);
        }
        av_dict_set_int(&opts, "rw_timeout", 2000, 0);
    ...
    for (rt->server_type = RTSP_SERVER_RTP;;) {
         ...
         * and try DESCRIBE while reply with redirect location
         */
        if(!send_rtsp_options) {
            break;
        }
        /* end add by H.Kernel 2018-06-19 for HIK rtsp server*/
        cmd[0] = 0;
        if (rt->server_type == RTSP_SERVER_REAL)
            av_strlcat(cmd,
                       /*
        ...
        if (reply->status_code != RTSP_STATUS_OK) {
            ...
            if (reply->status_code >=300 && reply->status_code < 400 && s->iformat) {
                if(0 == strlen(reply->location)) {
                    /* use the old url try DESCRIBE*/
                    av_strlcpy(reply->location, s->url, sizeof(reply->location));
                    send_rtsp_options = 0;
                    av_log(s, AV_LOG_INFO, "Status %d: options no location  location :%s Redirecting to old url DESCRIBE %s\n",
                           reply->status_code,
                           reply->location,
                           s->url);
                }
                       ...
                       int64_t wait_end)
{
    ...
    case RTSP_LOWER_TRANSPORT_TCP:
        ...
        len = ff_rtsp_tcp_read_packet_fast(s, rtsp_st, rt->recvbuf, RECVBUF_SIZE);
...
int ff_rtsp_fetch_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (rt->cur_transport_priv) {
        ...
        /* Begin Add by H.Kernel for rtsp recv and pasrse by tcp */
        else if(AVERROR_INVALIDDATA == ret) {
            av_log(s, AV_LOG_WARNING,
                "recv invaliddata packet,so end of the stream.\n");
            return AVERROR_EOF;
        /* End Add by H.Kernel for rtsp recv and pasrse by tcp */
        } else
            rt->cur_transport_priv = NULL;
    }
