...
static int rtsp_read_play(AVFormatContext *s)
{
    ...
    if (!(rt->server_type == RTSP_SERVER_REAL && rt->need_subscription)) {
        ...
        if (rt->state == RTSP_STATE_PAUSED) {
             ...
             */
            if((NULL != rt->play_scale)&&(NULL != rt->play_speed)) {
...
static int ff_read_interleaved_rtsp_packet(AVFormatContext *s, RTSPStream **prtsp_st,
                            uint8_t *buf, int buf_size) {
    RTSPState *rt = s->priv_data;
    int id, len, i;
    RTSPStream *rtsp_st;

    if(rt->tcp_recv_size < 4) {
        return 0;/* need more data */
    }
    id  = rt->tcp_recv_buf[1];
    len = AV_RB16(&rt->tcp_recv_buf[2]);

    //av_log(s, AV_LOG_TRACE, "[rtsp]:tcp_read_interleaved_rtsp_packet,id=%d len=%d\n", id, len);

    if (len < 8) {
        return 0;/* need more data */
    }
    if (len > buf_size) {
        av_log(s, AV_LOG_ERROR, "[rtsp]:tcp_read_interleaved_rtsp_packet,"
                            "the packet lens:[%d] is bigger than buf_size:[%d],so end the stream.\n",
                            len,buf_size);
        return AVERROR_EOF;/* error packet size data */
    }
    if(rt->tcp_recv_size < (len + 4)) {
        return 0;/* need more data */
    }

    memcpy(buf,&rt->tcp_recv_buf[4],len);

    rt->tcp_recv_size -= (len + 4);
    memmove(&rt->tcp_recv_buf[0],&rt->tcp_recv_buf[len+4],rt->tcp_recv_size);
    ...
    av_log(s, AV_LOG_ERROR, "[rtsp]:tcp_read_interleaved_rtsp_packet,"
                            "the id:[%d] is not between[%d]~[%d],so drop the unkonw data.\n",
                            id,rtsp_st->interleaved_min,rtsp_st->interleaved_max);
    
    /**
     * add  by H.Kernel 2020-01-2 for huawei vcn send metadata with rtsp tcp channel
     **/
    //return AVERROR_EOF;/* error packet size data */
    return 0;
}
int ff_rtsp_skip_packet_from_buffer(AVFormatContext *s)
{
    RTSPState *rt = s->priv_data;
    int id, len,ret;
    int attampt = 0;
...
redo:
        ...
        if (ret < 0) {
            av_log(s, AV_LOG_ERROR, "skip the rtp packet,read socket fail,size:[%d] len:[%d].\n",
    ...
    if(rt->tcp_recv_size < 4) {
        goto redo;/* need more data */
    }
    id  = rt->tcp_recv_buf[1];
    len = AV_RB16(&rt->tcp_recv_buf[2]);

    av_log(s, AV_LOG_TRACE, "[rtsp]:tcp_skip_interleaved_rtsp_packet,id=%d len=%d\n", id, len);

    if (len < 8) {
        goto redo;/* need more data */
    }
    if (len > RTSP_TCP_RECV_MAX_SIZE) {
        av_log(s, AV_LOG_ERROR, "[rtsp]:tcp_skip_interleaved_rtsp_packet,"
                            "the packet lens:[%d] is bigger than buf_size:[%d],so end the stream.\n",
                            len,RTSP_TCP_RECV_MAX_SIZE);
        return -1;/* error packet size data */
    }
    if(rt->tcp_recv_size < (len + 4)) {
         goto redo;/* need more data */
    }
...
int ff_rtsp_tcp_read_packet_fast(AVFormatContext *s, RTSPStream **prtsp_st,
                            uint8_t *buf, int buf_size)
{
    RTSPState *rt = s->priv_data;
    int len, ret;
...
redo:
    ...
    if('$' == rt->tcp_recv_buf[0]) {
        ret = ff_read_interleaved_rtsp_packet(s,prtsp_st,buf,buf_size);
        if(ret == 0) {
            goto redo;
        }
        return ret;
    }
    /* 3.deal the rtsp packet */
    av_log(s, AV_LOG_DEBUG, "read packer fast,read reply from buffer,size:%d\n", rt->tcp_recv_size);
    ret = ff_rtsp_read_reply_from_buffer(s,&reply,NULL,0,NULL);
